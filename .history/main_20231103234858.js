...
function addNewCard() {
  ...
  if (studentName && imageUrl) {
    ...
    // Add the new student to the firstYearWizards array
    firstYearWizards.push({ name: studentName, imageUrl: imageUrl, house: randomHouse });
...
function sortStudentsByHouse(house) {
  ...
  if (house === "all") {
    ...
    sortedStudents = firstYearWizards.filter(student => student.house.toLowerCase() === house);
