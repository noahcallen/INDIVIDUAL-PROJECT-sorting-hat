...
cardsOnDom = (array) => {
  let domString = "";
    ...
    `<div class="card mb-3" style="max-width: 540px;">
      <div class="row g-0">
        <div class="col-md-4">
          <img src="${expelled.imageURL}" class="img-fluid rounded-start" alt="...">
        </div>
        <div class="col-md-8">
          <div class="card-body">
            <h5 class="card-title">${expelled.name}</h5>
            <p class="card-text">${expelled.house}</p>
            <button type="button" class="btn btn-danger id="delete--${expelled.id}">Delete</button>
          </div>
        </div>
      </div>
    </div>`;
