...
//expel obj from students array and moves it to expelled array
const app =document.querySelector('#app');

app.addEventListener('click', (taco) => {
  if (taco.target.classList.contains('expel')) { // Check for expel button class
    const id = taco.target.dataset.studentId; // Assuming you add a data attribute
    const index = students.findIndex(taco => taco.id === Number(id));
    students.splice(index, 1);
    expelled.push(students[index]);

    // Re-render student list first, then expelled list
    cardsOnDom(students);
    cardsOnDom2(expelled);

    taco.stopPropagation(); // Prevent bubbling up
  }
});
