...
app.addEventListener('click', (taco) => {
  console.log('Event Target:', taco.target);
  ...
  if (taco.target.id.includes('expel')) {
    ...
    if (index !== -1) {
      ...
      // Add debugging for DOM updates
      console.log('Before DOM update:', document.getElementById('app').innerHTML);
      console.log('Before DOM update:', document.getElementById('app2').innerHTML);

      cardsOnDom(students);
      cardsOnDom2(expelled);

      console.log('After DOM update:', document.getElementById('app').innerHTML);
      console.log('After DOM update:', document.getElementById('app2').innerHTML);
    } else {
      console.error('Student not found');
    }

    taco.stopPropagation();
  }
});
