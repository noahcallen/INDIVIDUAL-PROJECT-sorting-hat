...
function hideNonMatchingHouseCards(house) {
...
// Add click event listeners to each house button
houseButtons.forEach(button => {
  button.addEventListener("click", () => {
    const house = button.id; // Get the house type from the button id
    if (house === "allWizards") {
      ...
      hideNonMatchingHouseCards(house);
    }
  });
});
});
